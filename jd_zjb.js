...
// euper
//let InviterPin = ['', 'ob5uluTqqxkim0Mz5lIK1w==', 'i7My3ErIYoVy2fjiHNhoeg==', 'rTHoRtgQyTUSAa/ZIGgad/rjiNMKC4khK+NwHzL4bA0=', 'OQ+S7/wCR4DRCtWr99wWGw==', 'BOOWzXEQuXFwLXyL4O4Yhg==', 'N5sXHbGLgXflqADjaEVA5gmTtt2WMvSoxj2dlWMjlTs=', 'EdbcZVtSwNHeRHKiY0GE7A==', 'Kes3Gulo090COpiQkJP5iw=='];
let InviterPin = [
  {"use": "455979050-14287437","code": "F7o+2zi5L+gQIWuMgmnTBRosMAxTWqRP8DMXr9e39pM="},
  {"use": "jd_RbuUwXHBNnJR","code": "ob5uluTqqxkim0Mz5lIK1w=="},
  {"use": "jd_fMRfXcJHjROT","code": "i7My3ErIYoVy2fjiHNhoeg=="},
  {"use": "jd_5f4d5dcf6c686","code": "rTHoRtgQyTUSAa/ZIGgad"},
  {"use": "lman87","code": "OQ+S7/wCR4DRCtWr99wWGw=="},
  {"use": "变速箱2019","code": "BOOWzXEQuXFwLXyL4O4Yhg=="},
  {"use": "jd_7a84de20543eb","code": "N5sXHbGLgXflqADjaEVA5gmTtt2WMvSoxj2dlWMjlTs="},
  {"use": "a7028231","code": "EdbcZVtSwNHeRHKiY0GE7A=="},
  {"use": "宇众不同JZY","code": "Kes3Gulo090COpiQkJP5iw=="},
]
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      // }
      if (InviterPin && InviterPin.length) {
        for (let j = 0; j < InviterPin.length; j++) {
          console.log(`\n账号${$.UserName} 去助力 ${InviterPin[j].use}`)
          if ($.UserName === InviterPin[j].use) {
            console.log(`跳过，不能助力自己`)
            continue
          }
          $.delcode = false
          await help(InviterPin[j].code)
          await $.wait(2000)
          if ($.delcode) {
            InviterPin.splice(j, 1)
            j--
            continue
          }
        }
