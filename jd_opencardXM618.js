...
!(async () => {
  ...
  console.log('小米618瓜分京豆活动\n')
  $.activityId = "dz6b63d87149648e6ae145195"
  $.shareUuid = "39f681a5ac6c4730aec799728f1b8713"
  console.log(`入口:\nhttps://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/signBean/activity/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`)
  let shareUuidArr = ['39f681a5ac6c4730aec799728f1b8713','cfed797822d5477e8b5053763e16e027','d4fe748fb2204451aacca4651b03e89f','28549c260ddd4d26be9aa8c6e7555e7b']
...
async function run() {
  try {
    ...
    console.log('开通小米店铺会员');
    if($.allOpenCard == false){
          flag = true
          $.shopactivityId = ''
          $.joinVenderId = 1000004123
    ...
    $.log('分享好友: '+$.shareActive);
    if(!$.shareActive&&!$.outFlag){
    	flag=true;
    	await takePostRequest('分享好友');
    	await $.wait(parseInt(Math.random()*2000+1000,10));
    }
    $.mainActiveList=[1,2,3,4];
    $.log('浏览会场: '+$.mainActive);
    if(!$.mainActive&&!$.outFlag){
    	flag=true;
    	for(let i=0;i<$.mainActiveList.length;i++){
    		if($.runFalag==false)break;
    		$.visitSkuValue=$.mainActiveList[i];
  ...
  switch (type) {
    case 'isvObfuscator':
      ...
      case 'accessLogWithAD':
        ...
        let pageurl = `${domain}/dingzhi/xiaomi/signBean/activity/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
      ...
      case 'activityContent':
        url=`${domain}/dingzhi/xiaomi/signBean/activityContent`;
      ...
      case 'drawContent':
        ...
        break;
      case 'checkOpenCard':
        url = `${domain}/dingzhi/xiaomi/signBean/saveTask`;
      ...
      case '分享好友':
        url=`${domain}/dingzhi/xiaomi/signBean/saveTask`;
        body=`activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&taskType=8&taskValue=2`;
        break;
      case 'mainActive':
      	url=`${domain}/dingzhi/xiaomi/signBean/saveTask`;
      	body=`activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&taskType=12&taskValue=${$.visitSkuValue}`;
      	break;
      case 'startDraw':
      ...
      case '浏览商品':
      	...
      	break;
      case 'followShop':
        url = `${domain}/dingzhi/xiaomi/signBean/followShop`;
        body = `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.Pin)}&shareUuid=${$.shareUuid}`;
        break;
      case 'sign':
        url=`${domain}/dingzhi/xiaomi/signBean/saveTask`;
        body=`activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&taskType=0&taskValue=0`;
        break;
      case 'addCart':
        url=`${domain}/dingzhi/xiaomi/signBean/saveTask`;
        body=`activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&taskType=21&taskValue=`;
        break;
      case 'browseGoods':
      ...
      case 'activityContent':
        if(typeof res == 'object'){
          if(res.result && res.result === true){
            ...
            $.shareActive=res.data.shareActive.allStatus || false;
            $.followShop=res.data.followShop.allStatus || false;
            $.abcd=res.data.visitSku.data ;
            $.visitSku=res.data.visitSku.allStatus || false;
            $.mainActivelist=res.data.mainActive.data || [];
            $.mainActive=res.data.mainActive.allStatus || false;
            $.addSkulist=res.data.addSku.data || [];
            $.addSku=res.data.addSku.allStatus || false;
            $.sign=res.data.sign.allStatus || false;
            $.allopencard=res.data.opencard || false;
            $.guafen=res.data.guafen || false;
            $.actorUuid=res.data.actorUuid || '';
            $.assistState=res.data.assistState || 0;
          }else if(res.errorMessage){
            console.log(`${type} ${res.errorMessage || ''}`)
          }else{
            console.log(`${type} ${data}`)
        ...
        if(typeof res == 'object'){
          if(res.result && res.result === true){
            if(typeof res.data == 'object'){
              console.log('任务完成，获得积分：', res.data.taskScoreNum);
              let msg = ''
              let title = '抽奖'
...
function getPostRequest(url, body, method="POST") {
  ...
  if(url.indexOf('https://lzdz1-isv.isvjcloud.com') > -1){
    headers["Referer"] = `https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/signBean/activity/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
...
function getCk() {
  return new Promise(resolve => {
    let get = {
      url:`https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/signBean/activity/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
